...
SKIP = {"uv.lock"}
# raw/ 是逐字保存的原文，wiki/ 与 registry 是文献内容，不按本仓库的私人信息规则检查
...
def test_private_and_runtime_directories_are_ignored_and_untracked():
    ...
    for entry in ("/.notes/", "/.claude/", ".venv/", "data/state/kb_session.json"):
        assert entry in ignore, f".gitignore 缺少 {entry}"
    tracked = {p.relative_to(ROOT).as_posix() for p in tracked_files()}
    for prefix in (".notes/", ".claude/", "data/logs/acceptance-"):
        assert not any(p.startswith(prefix) for p in tracked), f"{prefix} 下仍有被跟踪的文件"
    assert ".kimi-code/mcp.json" not in tracked and ".obsidian/workspace.json" not in tracked
